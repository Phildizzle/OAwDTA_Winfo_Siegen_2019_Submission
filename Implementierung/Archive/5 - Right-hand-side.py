...
lb_one = math.floor(eps*math.exp((min(b_con/max_a)*math.pow(eps, 3))/(6*m)))    #Divide b_con by max_a since max_a reflects the maximum number of tickets permitted per customer and category. This requires an adjustment of right-hand side condition (see Remark 1.1 in Agrawal et al.)
