        ...
        if l != l_old and l != 0:
            ...
            if m % 2 == 1: #uneven m
                for i in range(int(((m + 1) / 2) - 1)):
                    sp_dyn_trans[i] = sp_dyn_trans[i] + sp_dyn_trans[i] * (0.1 / (((m + 1) / 2) - 1)) * (((m + 1) / 2) - (i + 1))   #Tuning parameter: 10% (Zuschlag)
                for i in range(int(((m + 1) / 2)), m):
                    sp_dyn_trans[i] = sp_dyn_trans[i] + sp_dyn_trans[i] * (0.1 / (((m + 1) / 2) - 1)) * (((m + 1) / 2) - (i + 1))   #Tuning parameter: 10% (Abschlag)
            if m % 2 == 0:
