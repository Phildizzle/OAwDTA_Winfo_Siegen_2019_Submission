...
def simulate(n, m, max_a):
    ...
    for j in range(n):
        ...
        for i in range(m):
            if m != 1:
                a[i][j] = max(np.random.normal(2*(1 + i/(2*(m-1))), 1*(m - i)),0) #np.random.poisson(lam = 0.4*(1 + i/(2*(m-1))))     #Capacity consumption of customer j for resource i: CHANGE HERE IF OTHER DISTRIBUTION DESIRED
            else: a[i][j] = max(np.random.normal(2, 1),0)#np.random.poisson(lam = 0.4)
            if a[i][j] > max_a:
            ...
            if m != 1:
                p[j] = max(p[j] + a[i][j]*int(round(np.random.normal(100*(1 - i/(2*(m-1))), 20*(m - i)))),0)  #5  #Objective function coefficient: CHANGE HERE IF OTHER DISTRIBUTION DESIRED
            else: p[j] = max(p[j] + a[i][j]*int(round(np.random.normal(100, 20))),0) #5
    return (p, a)
