...
for (i in 1:(length(One)/sim)) {
  ...
  Sim_Time_Avg[i] <- mean(Simulation_Time[(sim*(i-1)+1):(sim*i)])
  Perm_Time_Avg[i] <- mean(Permutation_Time[(sim*(i-1)+1):(sim*i)])
  Input_Time_Avg[i] <- mean(Input_Time[(sim*(i-1)+1):(sim*i)])
  OPT_f_Time_Avg[i] <- mean(OPT_f_Time[(sim*(i-1)+1):(sim*i)])
  OPT_i_Time_Avg[i] <- mean(OPT_i_Time[(sim*(i-1)+1):(sim*i)])
  Permute_Time_Avg[i] <- mean(Permute_Time[(sim*(i-1)+1):(sim*i)])
  One_Time_Avg[i] <- mean(One_Time[(sim*(i-1)+1):(sim*i)])
  Dyn_Time_Avg[i] <- mean(Dyn_Time[(sim*(i-1)+1):(sim*i)])
  Greedy_Time_Avg[i] <- mean(Greedy_Time[(sim*(i-1)+1):(sim*i)])
  Interval_Time_Avg[i] <- mean(Interval_Time[(sim*(i-1)+1):(sim*i)])
  One_Relaxed_Time_Avg[i] <- mean(One_Relaxed_Time[(sim*(i-1)+1):(sim*i)])
  Dyn_Relaxed_Time_Avg[i] <- mean(Dyn_Relaxed_Time[(sim*(i-1)+1):(sim*i)])
  WTP_Time_Avg[i] <- mean(WTP_Time[(sim*(i-1)+1):(sim*i)])
  Amazon_Time_Avg[i] <- mean(Amazon_Time[(sim*(i-1)+1):(sim*i)])
...
x_figure = c(10,20,50,100,200)
x_sc = c(10,200) #Change here
...
#par(xpd=TRUE)
#legend(-0.05, 0.1, legend=c("Dynamic", "One-Time", "Greedy", "Interval", "One-Time Relaxed", "Dynamic Relaxed", "WTP Learner", "Amazon Learner"),
...
round(Dyn_Avg*100,2)
round(One_Avg*100,2)
round(Greedy_Avg*100,2)
round(Interval_Avg*100,2)
round(Dyn_Relaxed_Avg*100,2)
round(One_Relaxed_Avg*100,2)
round(WTP_Avg*100,2)
round(Amazon_Avg*100,2)

#Partial Effects
summary(lm(Dyn_Avg ~ x_figure))
summary(lm(One_Avg ~ x_figure))
summary(lm(Greedy_Avg ~ x_figure))
...
#par(xpd=TRUE)
#legend(-0.08, (lb+(ub-lb)*0.25), legend=c("OPT Fractional", "OPT Integral", "Dynamic", "One-Time", "Greedy", "Interval", "One-Time Relaxed", "Dynamic Relaxed", "WTP Learner", "Amazon Learner"),
#       col=c("gray35", "darkmagenta", "darkgreen", "blue", "red", "darkorange", "mediumpurple", "gold", "hotpink", "saddlebrown"), lty=1, cex=0.8)
par(xpd=FALSE)

#Average Runtimes in Seconds
round(Dyn_Time_Avg,2)
round(One_Time_Avg,2)
round(Greedy_Time_Avg,2)
round(Interval_Time_Avg,2)
round(One_Relaxed_Time_Avg,2)
round(Dyn_Relaxed_Time_Avg,2)
round(WTP_Time_Avg,2)
round(Amazon_Time_Avg,2)
